/*
 * Copyright (c) 2018 flux
 */

package com.company.appr.core.process;
 ...
 */
@Component("flux_ProcModelDeployer")
public class ProcModelDeployer {
    ...
    public void deployAllModels() {
        String modelsDir = AppContext.getProperty("appr.processModelsDir");

        if (Strings.isNullOrEmpty(modelsDir)) {
            log.error("Models directory specified by the appr.processModelsDir property not found");
            return;
        }
    ...
    private void deployModel(ProcModel procModel) {
        ...
        processRepositoryManager.deployProcessFromXml(processXml, null, procModel);
    }
}
